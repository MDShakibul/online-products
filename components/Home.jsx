...
export default function Home({ products = [] }) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      return inCategory && (title.includes(query) || desc.includes(query));
    });
    ...
    <div className="grid gap-6 md:grid-cols-12">
      <aside className="md:col-span-3 lg:col-span-3 md:sticky md:top-20 md:max-h[calc(100vh-5rem)] md:overflow-auto self-start">
        <div className="rounded-xl border border-gray bg-card p-4 shadow-sm">
          ...
          {/* Category list */}
          <h3 className="mb-3 text-lg font-semibold text-primary">Categories</h3>
              ...
              return (
                <li key={c.name}>
                  <button
                    ...
                    className={`flex w-full cursor-pointer items-center justify-between gap-3 px-3 py-3 text-left transition ${
